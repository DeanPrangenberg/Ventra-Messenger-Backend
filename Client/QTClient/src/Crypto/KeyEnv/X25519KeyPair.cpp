...
void X25519KeyPair::GenerateNewKeyPair() {
  ...
  if (EVP_PKEY_keygen_init(ctx) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Keygen init failed"); }
  EVP_PKEY *p = nullptr;
  if (EVP_PKEY_keygen(ctx, &p) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Key generation failed"); }
  key_.reset(p);
...
std::string X25519KeyPair::getPublicPem() const {
  ...
  if (!PEM_write_bio_PUBKEY(bio, key_.get())) { BIO_free(bio); throw OpenSSLError("Write public PEM failed"); }
  return pemFromBio(bio);
}

std::string X25519KeyPair::getPrivatePem() const {
  BIO *bio = BIO_new(BIO_s_mem());
  if (!PEM_write_bio_PrivateKey(bio, key_.get(), nullptr, nullptr, 0, nullptr, nullptr)) { BIO_free(bio); throw OpenSSLError("Write private PEM failed"); }
  return pemFromBio(bio);
}
...
std::vector<uint8_t> X25519KeyPair::deriveSharedSecret(const std::vector<uint8_t> &peerPublic) const {
  ...
  if (!ctx) throw OpenSSLError("Context creation failed");
  if (EVP_PKEY_derive_init(ctx) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Derive init failed"); }
  auto peer = loadFromRaw(peerPublic, true);
  if (EVP_PKEY_derive_set_peer(ctx, peer.get()) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Set peer failed"); }
  size_t len = 0;
  if (EVP_PKEY_derive(ctx, nullptr, &len) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Size derive failed"); }
  std::vector<uint8_t> secret(len);
  if (EVP_PKEY_derive(ctx, secret.data(), &len) <= 0) { EVP_PKEY_CTX_free(ctx); throw OpenSSLError("Secret derive failed"); }
  EVP_PKEY_CTX_free(ctx);
  return secret;
}
...
std::string X25519KeyPair::encodeBase64(const std::vector<uint8_t> &data) {
  ...
  if (BIO_write(bio, data.data(), data.size()) <= 0 || BIO_flush(bio) <= 0) { BIO_free_all(bio); throw OpenSSLError("Base64 encode failed"); }
  return pemFromBio(mem);
}

std::vector<uint8_t> X25519KeyPair::decodeBase64(const std::string &b64) {
  BIO *b64f = BIO_new(BIO_f_base64()); BIO *mem = BIO_new_mem_buf(b64.data(), b64.size());
  BIO_set_flags(b64f, BIO_FLAGS_BASE64_NO_NL);
  BIO *bio = BIO_push(b64f, mem);
  std::vector<uint8_t> buf(b64.size()); int len = BIO_read(bio, buf.data(), buf.size());
  BIO_free_all(bio);
