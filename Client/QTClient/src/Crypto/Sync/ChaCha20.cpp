...
namespace Crypto {
  bool ChaCha20::encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key, const uint8_t *iv,
    ...
    if (1 != EVP_EncryptUpdate(ctx, nullptr, &len, nullptr, 0)) {
      EVP_CIPHER_CTX_free(ctx);
      return false;
    }
  ...
  bool ChaCha20::decrypt(const uint8_t *ciphertext, size_t ciphertext_len,
    ...
    // Set associated data (optional, can be NULL if not needed)
    if (1 != EVP_DecryptUpdate(ctx, nullptr, &len, nullptr, 0)) {
      EVP_CIPHER_CTX_free(ctx);
      return false;
    }
